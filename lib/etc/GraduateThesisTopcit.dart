...
class _GraduateThesisTopcitDialogState extends State<GraduateThesisTopcitDialog> {
  ...
  Future<void> checkExistedData() async {
    _isCheckedPass = await isExisted(db, auth, "graduateThesisTopcitDialog", "졸업논문TOPCIT");
                          ...
                          Transform.scale(
                            ...
                            child: Checkbox(
                              ...
                              onChanged: (value) async {
                                // 체크하는 경우
                                if(value == true){
                                  await setSubject(db, auth, "graduateThesisTopcitDialog", "졸업논문TOPCIT", 0, "0-0");
                                }
                                // 체크를 해제하는 경우
                                else {
                                  await deleteSubject(db, auth, "graduateThesisTopcitDialog", "졸업논문TOPCIT");
                                }
                                setState(() {
                                  // 리빌드
                                });
