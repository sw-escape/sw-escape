...
class _LowestGraduateGPADialogState extends State<LowestGraduateGPADialog> {
  ...
  Future<void> checkExistedData() async {
    _isChecked = await isExisted(db, auth, "lowestGraduateGPADialog", "최저졸업평점");
    _isChecked ??= false;
  }
                          ...
                          Transform.scale(
                            ...
                            child: Checkbox(
                              ...
                              onChanged: (value) async {
                                // 체크하는 경우
                                if(value == true) {
                                  await setSubject(db, auth, "lowestGraduateGPADialog", "최저졸업평점", 0, "0-0");
                                }
                                // 체크를 해제하는 경우
                                else {
                                  await deleteSubject(db, auth, "lowestGraduateGPADialog", "최저졸업평점");
                                }
                                setState(() {
                                  // 리빌드
                                });
